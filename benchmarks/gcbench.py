...
USAGE = """gcbench [num_repetitions] [--depths=N,N,N..] [--threads=N] [--multiple=N]"""
ENABLE_THREADS = True
...
kStretchTreeDepth    = 15
kLongLivedTreeDepth  = 14
kArraySize  = 50000
kMinTreeDepth = 4
kMaxTreeDepth = 14
multiple = 8
...
def num_iters(i):
    ...
    return multiple * tree_size(kStretchTreeDepth) // tree_size(i) // 2;
...
def entry_point(argv):
    ...
    for arg in argv[1:]:
        if arg.startswith('--threads='):
            ...
            arg = arg[len('--multiple='):]
            try:
                multiple = int(arg)
                kArraySize = kArraySize * multiple
            except ValueError:
                return argerror()
        else:
            try:
