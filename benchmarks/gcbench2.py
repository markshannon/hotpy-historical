...
USAGE = """gcbench [num_repetitions] [--depths=N,N,N..] [--threads=N] [--fast]"""
ENABLE_THREADS = True
...
def main(depths=DEFAULT_DEPTHS, threads=0, fast_version = False):
    ...
    if fast_version:
        print(" Creating a long-lived string of %d characters" % (kArraySize*4))
        array = 'abcd' * kArraySize
    else:
    ...
    if fast_version:
        if long_lived_tree is None or array[1024] != 'a':
            raise Failed
    else:
...
def entry_point(argv):
    ...
    for i in range(repeatcount):
        main(depths, threads, fast_version)
    return 0
